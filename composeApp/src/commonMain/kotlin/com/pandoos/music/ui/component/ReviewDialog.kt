...
import pandoos.composeapp.generated.resources.*
...
fun ReviewDialog(
    ...
    AlertDialog(
        ...
        confirmButton = {
            TextButton(onClick = {
                onDoneReview.invoke()
                uriHandler.openUri("https://github.com/maxrave-dev/Pandoos")
            }) {
                Text(
        ...
        title = {
            Text(
                stringResource(Res.string.enjoying_pandoos),
        ...
        text = {
            Text(
                buildAnnotatedString {
                    append(stringResource(Res.string.if_you_enjoy_using_pandoos_star_pandoos_on_github_or_leave_a_review_on))
                    withLink(
                        LinkAnnotation.Url(
                            "https://www.producthunt.com/products/pandoos",
                            TextLinkStyles(style = SpanStyle(textDecoration = TextDecoration.Underline, color = seed)),
                        ) {
                            onDoneReview.invoke()
                            onDismissRequest.invoke()
                            uriHandler.openUri("https://www.producthunt.com/products/pandoos")
                        },
                    ) {
                        append(" ProductHunt")
                    }

 ...
 * - pandoos://open-app?url=<encoded_url>  (redirected from website)
 * - pandoos://watch?v=VIDEO_ID            (direct scheme)
 * - pandoos://playlist?list=PLAYLIST_ID   (direct scheme)
 * - pandoos://channel/CHANNEL_ID          (direct scheme)
 * - pandoos://album?id=ALBUM_ID           (direct scheme)
 * - https://pandoos.org/app/...            (web URL passed via args)
 */
object DesktopDeepLinkHandler {
    private const val TAG = "DesktopDeepLinkHandler"

    private val pendingUriFile: File by lazy {
        File(System.getProperty("java.io.tmpdir"), "pandoos_pending_deeplink.txt")
    }
     ...
     *
     * Conversion rules:
     * 1. pandoos://open-app?url=<encoded_url>
     *    → Extract the `url` param and use it as intent data
     *
     * 2. pandoos://watch?v=xxx, pandoos://playlist?list=xxx, etc.
     *    → Convert to https://pandoos.org/app/watch?v=xxx format
     *      so App.kt handles it uniformly via the pandoos.org branch
     *
     * 3. https://pandoos.org/app/... or YouTube URLs
     *    → Pass through as-is
     */
    private fun parseToIntent(uri: String): GenericIntent {
        ...
        val actualUri = when {
            // pandoos://open-app?url=<encoded_url>
            parsed.scheme == "pandoos" && parsed.host == "open-app" -> {
                val urlParam = parsed.getQueryParameter("url")
                if (urlParam != null) {
                    Logger.d(TAG, "Extracted URL from open-app: $urlParam")
                    Uri.parse(urlParam)
                } else {
                    // pandoos://open-app without params → just open the app, no navigation
                    Logger.d(TAG, "open-app without URL param, just opening app")
                    null
                }
            }

            // pandoos://watch?v=xxx → https://pandoos.org/app/watch?v=xxx
            // pandoos://playlist?list=xxx → https://pandoos.org/app/playlist?list=xxx
            // pandoos://channel/UCxxx → https://pandoos.org/app/channel/UCxxx
            // pandoos://album?id=xxx → https://pandoos.org/app/album?id=xxx
            parsed.scheme == "pandoos" && parsed.host != null -> {
                val host = parsed.host!!
                val query = parsed.query?.let { "?$it" } ?: ""
                val pathSuffix = parsed.pathSegments.joinToString("/").let {
                    if (it.isNotEmpty()) "/$it" else ""
                }
                val convertedUrl = "https://pandoos.org/app/$host$pathSuffix$query"
                Logger.d(TAG, "Converted pandoos:// to: $convertedUrl")
                Uri.parse(convertedUrl)
            }

            // https://pandoos.org/app/... or YouTube URLs → pass through
            else -> parsed
        }

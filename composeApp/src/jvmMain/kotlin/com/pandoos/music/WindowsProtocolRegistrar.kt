...
/**
 * Registers the "pandoos://" custom URI protocol handler in Windows Registry
 * under HKEY_CURRENT_USER (no admin rights required).
 *
 * Registry structure:
 * ```
 * HKCU\Software\Classes\pandoos
 *     (Default) = "URL:Pandoos Protocol"
 *     URL Protocol = ""
 *     \DefaultIcon
 *         (Default) = "\"<exe_path>\",0"
 *     \shell\open\command
...
object WindowsProtocolRegistrar {
    ...
    private const val SCHEME = "pandoos"
    ...
    fun register() {
        ...
        try {
            ...
            Logger.d(TAG, "Registering pandoos:// protocol handler -> $exePath")

            // Main key with protocol description
            regAdd(REG_KEY, null, "URL:Pandoos Protocol")
    ...
    private fun isAlreadyRegistered(currentExePath: String): Boolean {
        return try {
            ...
            // Registry stores path with quotes: "C:\path\to\Pandoos.exe" "%1"
    ...
    private fun resolveExePath(): String? {
        ...
        //   <app>/Pandoos.exe
        // So we go: java.home → parent (runtime) → parent (app) → Pandoos.exe
        ...
        if (appDir != null) {
            val exeFile = java.io.File(appDir, "Pandoos.exe")
            if (exeFile.exists()) {
